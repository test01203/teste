// cuevana provider for Nuvio  --  ported from LIVE FilmPlus config (github.com/sonicko16)
...
// ===== Cuevana / PelisPlusHD  (esplay GraphQL "hd" method) =================
// Faithful to searchscripts.json -> scripts.hd. Mirror sites (rotate often):
//   cuevanahd.net, pelisplushd.to, pelisplushd.lat  (share api.esplay.one).
var SITES = ['https://cuevanahd.net', 'https://pelisplushd.to'];
var GQL_HOSTS = ['https://api.esplay.one/graphql'];

var Q_SLUG = 'query movie_slug($slug: String) { show(slug: $slug, type: "movie") { id title type slug __typename } }';
var Q_VIDS = 'query queryVideos($itemId: String!) { videos(itemId: $itemId) { language url quality server __typename } }';
var Q_EP   = 'query episode($showSlug: String!, $seasonNumber: Int!, $number: Int!) { episodeInfoByNumber(showSlug: $showSlug, seasonNumber: $seasonNumber, number: $number) { episode { id title __typename } __typename } }';

async function gql(op, query, vars, referer) {
  for (var i = 0; i < GQL_HOSTS.length; i++) {
    try {
      var r = await fetch(GQL_HOSTS[i], {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'User-Agent': UA,
          'Origin': referer.replace(/\/$/, ''), 'Referer': referer + '/' },
...
async function findSlug(site, title) {
  var html = await txt(site + '/search?q=' + encodeURIComponent(title), site);
  if (!html) html = await txt(site + '/?s=' + encodeURIComponent(title), site);
  var m = html.match(/(?:pelicula|serie|movies|series|ver)\/([a-z0-9\-]+)/i);
...
async function getStreams(tmdbId, type, season, episode) {
  try {
    ...
    if (!meta) return [];
    var raw = [];
    for (var s = 0; s < SITES.length && !raw.length; s++) {
      var site = SITES[s];
      var slug = await findSlug(site, meta.title);
      var d = await gql('movie_slug', Q_SLUG, { slug: slug }, site);
      var id = d && d.show && d.show.id;
      if (!id) continue;
      var itemId = id;
      if (type === 'tv') {
        var ep = await gql('episode', Q_EP, { showSlug: slug, seasonNumber: Number(season), number: Number(episode) }, site);
        if (ep && ep.episodeInfoByNumber && ep.episodeInfoByNumber.episode) itemId = ep.episodeInfoByNumber.episode.id;
      }
      var v = await gql('queryVideos', Q_VIDS, { itemId: String(itemId) }, site);
      var vids = (v && v.videos) || [];
      for (var i = 0; i < vids.length; i++) {
        var solved = await resolveEmbed(vids[i].url, site);
        var lang = vids[i].language || '';
        solved.forEach(function (r) { r.server = (r.server || '') + (lang ? ' · ' + lang : ''); });
        raw = raw.concat(solved);
      }
    }
    return toStreams('Cuevana', raw, SITES[0]);
  } catch (e) { return []; }
}
