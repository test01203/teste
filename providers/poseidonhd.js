// poseidonhd provider for Nuvio  --  ported from LIVE FilmPlus config (github.com/sonicko16)
...
// ===== PoseidonHD  ("biz" method, poseidonhd2.co) ==========================
// Faithful to searchscripts.json -> scripts.biz  (regex: videos":(.+?),"downloads)
var BASE = 'https://poseidonhd2.co';

function pageEmbeds(html) {
  var urls = [];
  // 1) the "videos":[ ... ],"downloads" JSON blob (biz.regex)
  var blob = (html.match(/videos"\s*:\s*(\[.*?\])\s*,\s*"downloads/s) || [])[1] || html;
  var m, re = /"(?:result|url|link|embed)"\s*:\s*"([^"]+)"/gi;
  while ((m = re.exec(blob)) !== null) urls.push(m[1].replace(/\\\//g, '/').replace(/\\u002F/gi, '/'));
  // 2) any iframe/embed url as fallback
  var re2 = /(https?:\/\/[^\s"'<>]*?(?:embed|\/e\/|\/v\/|player|okru|stream|dood|filemoon|voe)[^\s"'<>]*)/gi;
...
async function tryPage(url) {
  var html = await txt(url, BASE);
  if (!html || html.length < 500) return [];
  var raw = [], embeds = pageEmbeds(html);
  for (var i = 0; i < embeds.length; i++) raw = raw.concat(await resolveEmbed(embeds[i], url));
  return raw;
}
...
async function getStreams(tmdbId, type, season, episode) {
  try {
    ...
    var oslug = slugify(meta.original);
    var raw = [];
    var candidates = [];
    if (type === 'tv') {
      candidates.push(BASE + '/serie/' + slug + '/temporada/' + season + '/capitulo/' + episode);
      candidates.push(BASE + '/episodio/' + slug + '-' + season + 'x' + episode);
      if (oslug !== slug) candidates.push(BASE + '/serie/' + oslug + '/temporada/' + season + '/capitulo/' + episode);
    } else {
      candidates.push(BASE + '/pelicula/' + slug);
      if (oslug !== slug) candidates.push(BASE + '/pelicula/' + oslug);
    }
    for (var i = 0; i < candidates.length && !raw.length; i++) raw = raw.concat(await tryPage(candidates[i]));
    return toStreams('PoseidonHD', raw, BASE);
  } catch (e) { return []; }
}
